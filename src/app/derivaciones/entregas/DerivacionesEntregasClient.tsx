...
export default function DerivacionesEntregasClient({ data }: { data: Registro[] }) {
  const pendings = data
            ...
            {pendings.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  <div className="flex flex-col items-center justify-center gap-2">
                    ...
                    <p>No hay entregas de derivación pendientes en la agenda.</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              pendings.map((r) => (
                <TableRow key={r.identificador} className="group hover:bg-muted/30 transition-colors border-border/20 cursor-pointer">
                  ...
                  <TableCell className="text-muted-foreground py-4">{r.sucursal}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
