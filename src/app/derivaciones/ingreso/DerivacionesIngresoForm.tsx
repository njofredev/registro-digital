...
import { createDerivacion } from '../actions';
import { Save, User, Activity, CalendarDays, Box, ClipboardList, Check, AlertCircle, ArrowRight, Plus, X } from 'lucide-react';
...
export default function DerivacionesIngresoForm({ nextId, options }: { nextId: number; options: any }) {
  ...
  // Para derivaciones el doctor exclusivo es Antonio Alvear
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const patientName = (formData.get('nombre_paciente') as string) || 'Sin nombre';
      const result = await createDerivacion(formData);

      if (result.success) {
        setCreatedId(result.identificador || nextId);
        setCreatedPatient(patientName);
        form.reset();
        setSelectedTeeth([]);
        router.refresh(); 
        setShowSuccessModal(true);
      } else {
        setErrorMsg(result.error || 'Ocurrió un error inesperado al guardar la derivación.');
        window.scrollTo({ top: 0, behavior: 'smooth' });
      }
    } catch (err: any) {
      console.error("Exception during createDerivacion:", err);
      setErrorMsg('No se pudo conectar con el servidor para guardar la derivación.');
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } finally {
    ...
    <>
      <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in duration-700">
        ...
        {errorMsg && (
          <div className="bg-destructive/10 border border-destructive/20 text-destructive p-4 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4 duration-500 shadow-sm">
            ...
            <div>
              <h4 className="font-bold">Error al Registrar Derivación</h4>
      ...
      <div className="bg-card/40 backdrop-blur-xl border border-border/50 rounded-2xl p-6 shadow-lg">
        <h3 className="font-semibold text-lg flex items-center gap-2 mb-6 text-foreground/90 border-b border-border/40 pb-4">
          ...
          Apertura de Caso de Derivación
        </h3>
      ...
      <div className="bg-card/40 backdrop-blur-xl border border-border/50 rounded-2xl p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="space-y-3">
            ...
            <Select name="doctor" defaultValue="Antonio Alvear">
      ...
      <div className="bg-card/40 backdrop-blur-xl border border-border/50 rounded-2xl p-6 shadow-lg">
        ...
        <div className="space-y-3">
          <Label htmlFor="asunto_detalles" className="text-muted-foreground font-semibold text-xs uppercase tracking-wider flex items-center gap-2">
            ...
            Detalles y Observaciones de la Derivación
          </Label>
        ...
        <div className="pt-6 flex justify-end">
          <Button type="submit" disabled={isSubmitting} size="lg" className="w-full md:w-auto px-10 h-14 text-base font-semibold rounded-xl shadow-lg hover:shadow-primary/20 transition-all gap-2">
            ...
            {isSubmitting ? 'Registrando derivación...' : 'Guardar Nueva Derivación'}
          </Button>
        </div>
      </form>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-card border border-border/60 shadow-2xl rounded-3xl p-6 sm:p-8 max-w-md w-full relative overflow-hidden animate-in zoom-in-95 duration-300">
            {/* Background Glow */}
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  El caso se guardó correctamente en la base de datos de derivaciones.
                </p>
              </div>

              {/* Summary Card */}
              <div className="w-full bg-secondary/30 border border-border/40 rounded-2xl p-4 text-left space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">Identificador:</span>
                  <span className="font-bold text-foreground">#{createdId}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">Paciente:</span>
                  <span className="font-semibold text-foreground truncate max-w-[200px]" title={createdPatient}>
                    {createdPatient}
                  </span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">Doctor:</span>
                  <span className="font-semibold text-primary">Antonio Alvear</span>
                </div>
              </div>
              ...
              <div className="w-full flex flex-col sm:flex-row gap-2.5 pt-2">
                <button
                  type="button"
                  onClick={() => setShowSuccessModal(false)}
                  className="flex-1 h-11 px-4 rounded-xl bg-primary text-primary-foreground font-semibold text-xs flex items-center justify-center gap-1.5 shadow-md hover:bg-primary/90 transition-all cursor-pointer"
                >
                  <Plus className="w-4 h-4" />
                  Ingresar otra
                </button>
                <Link
                  href="/derivaciones/visualizador"
                  className="flex-1 h-11 px-4 rounded-xl bg-secondary/80 hover:bg-secondary text-foreground font-semibold text-xs flex items-center justify-center gap-1.5 border border-border/40 transition-all"
                >
                  Ver en Buscador
