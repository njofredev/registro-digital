...
export async function getRadiologiaRegistros() {
  try {
    return await db.radiologia.findMany({
      where: {
        AND: [
          { nombre: { not: null } },
          { nombre: { not: '' } }
    ...
    console.error("Error in getRadiologiaRegistros:", error);
    throw new Error(error.message || "Error al obtener registros de radiología");
  }
}

export async function createRadiologiaRegistro(formData: any) {
  try {
    const data = {
    ...
    const newRecord = await db.radiologia.create({
      data,
    });

    revalidatePath('/radiologia');
    return { success: true, record: newRecord };
  } catch (error: any) {
    console.error("Error in createRadiologiaRegistro:", error);
    return { success: false, error: error.message || "Error al crear el registro de radiología" };
  }
}

export async function updateRadiologiaRegistro(id: number | string, formData: any) {
  try {
    ...
    const updated = await db.radiologia.update({
      where: { id: parsedId },
      data,
    });
    ...
    console.error("Error in updateRadiologiaRegistro:", error);
    return { success: false, error: error.message || "Error al actualizar el registro de radiología" };
  }
}

export async function deleteRadiologiaRegistro(id: number | string) {
  try {
    const parsedId = typeof id === 'string' ? parseInt(id, 10) : id;
    if (isNaN(parsedId)) {
      throw new Error("ID inválido");
    ...
    console.error("Error in deleteRadiologiaRegistro:", error);
    return { success: false, error: error.message || "Error al eliminar el registro de radiología" };
  }
}
