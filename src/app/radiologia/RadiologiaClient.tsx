...
import { createRadiologiaRegistro, updateRadiologiaRegistro, deleteRadiologiaRegistro } from './actions';
...
export default function RadiologiaClient({ initialData }: { initialData: any[] }) {
  ...
  useEffect(() => {
    if (searchParams.get('ingreso') === 'true') {
      ...
      router.replace(`/radiologia?${params.toString()}`);
    }
  }, [searchParams]);

  const handleDelete = async (id: number) => {
    if (typeof window !== 'undefined' && !window.confirm('¿Estás seguro de que deseas eliminar este registro de radiología?')) return;
    const res = await deleteRadiologiaRegistro(id);
    if (res.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditMode && currentId) {
      const res = await updateRadiologiaRegistro(currentId, payload);
      ...
      const res = await createRadiologiaRegistro(payload);
      if (res.success && res.record) {
