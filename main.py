...
with st.sidebar:
    ...
    st.caption("Gestión integral de laboratorio.")
    st.divider()
...
# --- TAB 1: INGRESO (Igual a la versión anterior) ---
with tabs[0]:
...
# --- TAB 2 & 3: VISUALIZADOR Y EDICIÓN (Manteniendo funcionalidad restaurada) ---
with tabs[1]:
    ...
    if not df_e_raw.empty:
        ...
        if sel_e != "...":
            ...
            with st.form("form_ed_completo"):
                c1, c2, c3 = st.columns(3); ed_pac = c1.text_input("Paciente", value=str(d['nombre_paciente'])); ed_est = c2.selectbox("Estado", OPCIONES_ESTADO, index=OPCIONES_ESTADO.index(d['estado']) if d['estado'] in OPCIONES_ESTADO else 0); ed_doc = c3.selectbox("Doctor", OPCIONES_DOCTOR, index=OPCIONES_DOCTOR.index(d['doctor']) if d['doctor'] in OPCIONES_DOCTOR else 0)
                ...
                ed_f_dis = c7.date_input("Fecha Diseño", value=to_dt(d['fecha_diseno'])); ed_f_fre = c8.date_input("Fecha Fresado", value=to_dt(d['fecha_fresado'])); ed_f_ent = c9.date_input("Fecha Entrega", value=to_dt(d['fecha_entrega']))
...
# --- TAB 4: DASHBOARD (RESTAURADO CON REPORTES AVANZADOS) ---
with tabs[3]:
    st.header("📈 Dashboard de Gestión")
    st.caption("Análisis avanzado de productividad, materiales y tendencias temporales.")
    df_db = cargar_datos()
    if not df_db.empty:
        k1, k2, k3, k4 = st.columns(4)
        k1.metric("Total Casos", len(df_db))
        k2.metric("Tons con mayor carga", df_db['Tons a cargo'].mode()[0] if not df_db['Tons a cargo'].mode().empty else "-")
        k3.metric("Material Preferido", df_db['Material'].mode()[0] if not df_db['Material'].mode().empty else "-")
        k4.metric("Día Pico Ingresos", str(df_db['Fecha de ingreso'].mode()[0]) if not df_db['Fecha de ingreso'].empty else "-")
        
        st.divider()
        g1, g2 = st.columns(2)
        g1.plotly_chart(px.bar(df_db['Estado'].value_counts().reset_index(), x='Estado', y='count', color='Estado', title="Trabajos por Estado"), use_container_width=True)
        g2.plotly_chart(px.pie(df_db, names='Material', title="Uso de Materiales", hole=0.4), use_container_width=True)
...
with tabs[4]:
    ...
    st.caption("Agenda de despachos pendientes organizada por prioridad de fecha.")
    df_p = cargar_datos()
    if not df_p.empty:
...
# --- TAB 6: EXPORTACIÓN (RESTAURADO CON FILTROS Y EXCEL/CSV) ---
with tabs[5]:
    st.header("📥 Exportar Datos")
    st.caption("Filtre la información y descargue reportes detallados en formato Excel o CSV.")
    df_ex = cargar_datos()
    if not df_ex.empty:
        with st.expander("🔍 Filtrar Reporte para Exportación", expanded=True):
        ...
        st.write(f"📊 **Registros en el reporte:** {len(df_final)}")
        c_d1, c_d2 = st.columns(2)
        
        # Descarga CSV
        c_d1.download_button("📥 Descargar CSV", df_final.to_csv(index=False).encode('utf-8-sig'), "lab_tabancura_reporte.csv", "text/csv", use_container_width=True)
        
        # Descarga Excel
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as wr:
            df_final.to_excel(wr, index=False, sheet_name='Reporte')
        c_d2.download_button("📊 Descargar Excel", output.getvalue(), "lab_tabancura_reporte.xlsx", use_container_width=True)