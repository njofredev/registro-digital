...
with st.sidebar:
    ...
    st.divider()
    st.info("💡 **Consejo:** Use los tooltips (?) para obtener ayuda sobre cada campo.")
...
if engine is None:
    st.error("🔴 Error: Sin conexión a la base de datos.")
    st.stop()
...
with tabs[0]:
    ...
    st.caption("Complete la ficha técnica para iniciar un nuevo caso.")
    try:
    ...
    with st.form("form_ingreso", clear_on_submit=True):
        ...
        in_id = c1.number_input("N° Identificador", min_value=1, value=siguiente_id, help="ID correlativo automático.")
...
with tabs[1]:
    ...
    st.caption("Filtre y consulte el historial de trabajos.")
    busq = st.text_input("🔍 Buscar:", placeholder="Escriba aquí...")
    df_v = cargar_datos()
    if not df_v.empty:
        if busq:
...
# --- TAB 3: EDICIÓN (CON ELIMINACIÓN RESTAURADA) ---
with tabs[2]:
    st.header("✏️ Modificar Trabajo")
    st.caption("Actualice o elimine registros existentes.")
    df_e_raw = pd.read_sql("SELECT * FROM registros ORDER BY identificador DESC", engine)
    if not df_e_raw.empty:
        sel_e = st.selectbox("Seleccione caso:", ["..."] + [f"{r['identificador']} - {r['nombre_paciente']}" for _, r in df_e_raw.iterrows()])
        if sel_e != "...":
            ...
            with st.form("form_ed_completo"):
                c1, c2, c3 = st.columns(3)
                ed_pac = c1.text_input("Paciente", value=str(d['nombre_paciente']))
                ed_est = c2.selectbox("Estado", OPCIONES_ESTADO, index=OPCIONES_ESTADO.index(d['estado']) if d['estado'] in OPCIONES_ESTADO else 0)
                ed_doc = c3.selectbox("Doctor", OPCIONES_DOCTOR, index=OPCIONES_DOCTOR.index(d['doctor']) if d['doctor'] in OPCIONES_DOCTOR else 0)
                
                c4, c5, c6 = st.columns(3)
                ed_tons = c4.selectbox("Tons", OPCIONES_TONS, index=OPCIONES_TONS.index(d['tons_a_cargo']) if d['tons_a_cargo'] in OPCIONES_TONS else 0)
                ed_suc = c5.selectbox("Sucursal", OPCIONES_SUCURSAL, index=OPCIONES_SUCURSAL.index(d['sucursal']) if d['sucursal'] in OPCIONES_SUCURSAL else 0)
                ed_mat = c6.selectbox("Material", OPCIONES_MATERIAL, index=OPCIONES_MATERIAL.index(d['material']) if d['material'] in OPCIONES_MATERIAL else 0)
                
                c7, c8, c9 = st.columns(3)
                def to_dt(val): return pd.to_datetime(val).date() if val and str(val) != "None" else None
                ed_f_dis = c7.date_input("Fecha Diseño", value=to_dt(d['fecha_diseno']))
                ed_f_fre = c8.date_input("Fecha Fresado", value=to_dt(d['fecha_fresado']))
                ed_f_ent = c9.date_input("Fecha Entrega", value=to_dt(d['fecha_entrega']))
                
                c10, c11 = st.columns(2)
                ed_dis = c10.selectbox("Modalidad Diseño", OPCIONES_DISENO, index=OPCIONES_DISENO.index(d['diseno']) if d['diseno'] in OPCIONES_DISENO else 0)
                ed_blo = c11.selectbox("Bloques", OPCIONES_BLOQUES, index=OPCIONES_BLOQUES.index(d['bloques_usados']) if d['bloques_usados'] in OPCIONES_BLOQUES else 0)
            ...
            with st.expander("🗑️ Zona de Peligro"):
                ...
                if st.button("ELIMINAR TRABAJO PERMANENTEMENTE", use_container_width=True, type="primary"):
                    with engine.begin() as conn:
...
with tabs[3]:
    ...
    if not df_db.empty:
        ...
        k2.metric("Tons Activo", df_db['Tons a cargo'].mode()[0] if not df_db['Tons a cargo'].mode().empty else "-")
        k3.metric("Material Líder", df_db['Material'].mode()[0] if not df_db['Material'].mode().empty else "-")
        k4.metric("Día Pico", str(df_db['Fecha de ingreso'].mode()[0]) if not df_db['Fecha de ingreso'].empty else "-")
        st.divider()
        g1, g2 = st.columns(2)
        g1.plotly_chart(px.bar(df_db['Estado'].value_counts().reset_index(), x='Estado', y='count', color='Estado', title="Estados"), use_container_width=True)
        g2.plotly_chart(px.pie(df_db, names='Material', title="Materiales", hole=0.4), use_container_width=True)

# --- TAB 5: ENTREGAS ---
with tabs[4]:
    st.header("🗓️ Próximas Entregas")
    st.caption("Trabajos pendientes ordenados por fecha.")
    df_p = cargar_datos()
    if not df_p.empty:
...
with tabs[5]:
    ...
    st.caption("Descarga de reportes en Excel y CSV.")
    df_ex = cargar_datos()
    if not df_ex.empty:
        ...
        c_d1.download_button("Descargar CSV", df_final.to_csv(index=False).encode('utf-8-sig'), "reporte_lab.csv", use_container_width=True)
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as wr: df_final.to_excel(wr, index=False)
        c_d2.download_button("Descargar Excel", output.getvalue(), "reporte_lab.xlsx", use_container_width=True)